...
| Account | `owner@example.com` | `personal@example.com` |
...
- Verify the terminal `az` session with `az account show` → expect
  `Pay-As-You-Go` / `personal@example.com`. The terminal `az` and the MCP
...
(Requires the terminal `az` session to be `personal@example.com`, who is the SQL Entra admin.
