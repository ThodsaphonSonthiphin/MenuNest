...
`.agents/references/data-contracts.md`. Read it before writing —
...
```powershell
python ".agents/scripts/read_source.py" "<path-to-file>"
```
...
```powershell
python ".agents/scripts/read_source.py" "<path-to-file>" --out "<workdir>/source-dump.txt" --max-rows 0
```
