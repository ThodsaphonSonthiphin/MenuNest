...
**`key`** field (see `.agents/references/data-contracts.md`),
...
```powershell
python ".agents/scripts/tracking.py" add-columns --source "<file>" --key "#"
```
...
```powershell
python ".agents/scripts/tracking.py" writeback --source "<file>" --result "<path>/backlog_result.json" --key "#"
```
