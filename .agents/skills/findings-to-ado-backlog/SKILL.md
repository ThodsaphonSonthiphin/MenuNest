...
Full shapes live in `.agents/references/data-contracts.md` — read it once; do
...
```powershell
powershell -ExecutionPolicy Bypass -File ".agents/scripts/setup_check.ps1"
```
...
`.agents/scripts/read_source.py` for xlsx/csv; docs and pasted text are read
...
dotnet run ".agents/scripts/create-backlog.cs" -- "<workdir>/backlog_input.json"
```
...
Have **ado-writeback-tracking** drive `.agents/scripts/tracking.py`: first
