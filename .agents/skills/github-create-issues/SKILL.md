...
For the exact JSON shapes see `.agents/references/data-contracts.md`.
...
- **Python + requests + openpyxl** — run the setup checker if unsure:
  `powershell -ExecutionPolicy Bypass -File ".agents/scripts/setup_check_github.ps1"`
...
python ".agents/scripts/create_github_issues.py" `
