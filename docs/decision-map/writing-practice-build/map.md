...
- [Pending-correction visibility - how does the writer see which nights still need a correction pass?](tickets/pending-correction-visibility.md) — Status badge per row (pending/corrected) + tappable filter inside the History screen, built as a Syncfusion react-grid DataGrid; no count or badge on the nav.
