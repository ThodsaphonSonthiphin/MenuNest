...
`AZURE_TOKEN_CREDENTIALS=AzureCliCredential` is required — without it SqlClient picks the Visual Studio **work** account and the login fails against the personal-tenant server. Confirm first that `az account show` reports `Pay-As-You-Go` / `thodsaphonSP@hotmail.co.th`.
