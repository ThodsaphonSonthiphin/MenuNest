...
- **Resolved UI decisions (pre-flight, binding — override the example code where it differs):**
  ...
  - **Tab/day strips use Syncfusion** `Tab` (`@syncfusion/react-navigations`); implement the `SegmentedTabs` component as a thin Syncfusion-`Tab`-backed wrapper (controlled `value` + `onChange`), driving content from the parent. A custom layout-level control is acceptable **only** for a binary toggle (Map/List) **if** no Syncfusion equivalent is installed — confirm against the `.d.ts` first.
