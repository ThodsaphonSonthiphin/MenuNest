...
`AZURE_TOKEN_CREDENTIALS=AzureCliCredential` is required, or SqlClient picks the Visual Studio **work** account and the login fails. Confirm `az account show` reports `Pay-As-You-Go` / `thodsaphonSP@hotmail.co.th` first.
