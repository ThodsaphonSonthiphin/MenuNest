...
| 6 | Redo puts it back | **NOT RUN.** The run diverted into the #109 diagnosis at Step 4b and never returned to Redo. It stays unexercised in production — the one assertion in this runbook with no result. The row undone at Step 4b is still redoable until the month cut on 31 August; after that a fresh act in September is needed first. |  |
...
| 9 | A7 present, A8 absent | **PASS** — one round button in that corner, not two. The account screen's own `+` is present; the rail is absent. Operator-reported. This is the blast-radius check: nothing else in the run would have failed if the rail had leaked onto a page that never opted in (menunest-199). | 2026-08-30T02:2xZ |
