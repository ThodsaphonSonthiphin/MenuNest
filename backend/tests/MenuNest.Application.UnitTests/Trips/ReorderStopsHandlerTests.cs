...
public class ReorderStopsHandlerTests
{
    ...
    [Fact]
    public async Task Rejects_partial_or_mismatched_id_list()
    {
        using var fx = new HandlerTestFixture();
        var trip = Trip.Create(fx.User.Id, "t", new DateOnly(2026, 11, 1), 1, TravelMode.Drive);
        fx.Db.Trips.Add(trip);
        ...
        await FluentActions.Awaiting(() => new ReorderStopsHandler(fx.Db, fx.UserProvisioner.Object)
            .Handle(new ReorderStopsCommand(trip.Id, day.Id, new[] { s0.Id }), CancellationToken.None).AsTask())
