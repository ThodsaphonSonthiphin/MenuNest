...
public sealed class AddStopHandler : ICommandHandler<AddStopCommand, StopDto>
{
    ...
    public async ValueTask<StopDto> Handle(AddStopCommand c, CancellationToken ct)
    {
        ...
        return new StopDto(stop.Id, stop.TripPlaceId, stop.Sequence, stop.DwellMinutes, stop.TravelModeToReach, null, stop.IsVisited, Array.Empty<StopChecklistEntryDto>());
    }
}
